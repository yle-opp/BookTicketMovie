create database QLDatVeXemPhim;
use  QLDatVeXemPhim ;

DROP DATABASE QLDatVeXemPhim;


create table quocgia(
	id int auto_increment,
    ten_quocgia varchar(255),
    primary key(id)
);
INSERT INTO quocgia (id,ten_quocgia) VALUES
(1,'AMERICA'),
(2,'VIETNAM'),
(3,'KOREA'),
(4,'JAPAN'),
(5,'FRANCE'),
(6,'ENGLAND');
create table  khachhang(
	id int not null auto_increment,
    hoten nvarchar(255),
    matkhau nchar(10),
    taikhoan nchar(20),
    hinhdaidien text,
    email nvarchar(255),
    diachi nvarchar(255),
    dienthoai  nchar(10),
    gioitinh nchar(10),
    ngaysinh DATE ,
    primary key(id)
);
INSERT INTO khachhang (hoten, matkhau, taikhoan, hinhdaidien, email, diachi, dienthoai, gioitinh, ngaysinh) VALUES
('Nguyễn Văn Trung', '123456', 'nguyenvantrung', '1.jpg', 'nguyenvantrung@gmail.com', '525/7/14 lê trọng tấn, Tp.HCM', '0123456781', 'NAM', '1997-05-11') ,
('Trần Công Tùng', '123456', 'trancongtung', '2.jpg', 'trancongtung@gmail.com', '5 Lê Niệm, Tp.HCM', '0123456782', 'NAM', '1997-01-04'),
('Lê Ngọc Ánh', '123456', 'lengocanh', '3.jpg', 'lengocanh@gmail.com', '41 Trường Chinh, Tp.HCM', '0123456783', 'NỮ', '1981-09-12'),
('Nguyễn Giang Chương', '123456', 'nguyengiangchuong', '4.jpg', 'nguyengiangchuong@gmail.com', '400 Phan Văn Trị, Tp.HCM', '0123456784', 'NAM', '1993-01-03'),
('Nguyễn Thị Bình', '123456', 'nguyenthibinh', '5.jpg', 'nguyenthibinh@gmail.com', '20 Nguyễn Trãi, Tp.HCM', '0123456785', 'NỮ', '1972-12-05'),
('Phạm Trung Kiên', '123456', 'phamtrungkien', '6.jpg', 'phamtrungkien@gmail.com', '11 Lê Văn Quới, Tp.HCM', '0123456786', 'NAM', '2000-05-18'),
('Ngô Thị Kim Nhung', '123456', 'ngothikimnhung', '7.jpg', 'ngothikimnhung@gmail.com', '77/10 Nguyễn Thị Minh Khai, Tp.HCM', '0123456787', 'NỮ', '1994-01-10'),
('Nguyễn Minh Đức', '123456', 'nguyenminhduc', '8.jpg', 'nguyenminhduc@gmail.com', '146/19 Võ Thị Sáu, Tp.HCM', '0123456788', 'NAM', '2002-01-07'),
('Hạ Quang Minh', '123456', 'haquangminh', '9.jpg', 'haquangminh@gmail.com', '128/2 Nguyễn Thị Định, Tp.HCM', '0123456789', 'NAM', '1984-09-06'),
('Thanh Hanh', '123456', 'thanhhanh', '10.jpg', 'thanhhanh@gmail.com', '215 Thành Thái, Tp.HCM', '0123456780', 'NỮ', '1997-05-05') ;

create table roleskh(
   id_role int,
   tenrole varchar(25),
   primary key(id_role)
);
INSERT INTO roleskh (id_role, tenrole) VALUES
(1,'ROLE_USER'),
(2,'ROLE_ADMIN');

create table role_khachhang(
	role_id int,
    user_id int,
    primary key(role_id,user_id),
    foreign key(role_id) references roleskh(id_role),
    foreign key(user_id) references khachhang(id)
);
INSERT INTO role_khachhang(role_id, user_id) VALUES
(1,1),
(2,2),
(1,3),
(2,4),
(1,5),
(1,6),
(2,7),
(1,8),
(1,9),
(2,10);

create table  rap(
	id int not null auto_increment,
    tenrap nvarchar(255),
    diachi nvarchar(255),
    primary key(id)
    
);
INSERT INTO rap (tenrap, diachi) VALUES 
('RAP 1', 'CGV vicom Nguyễn Thị Nghĩa,tp.hcm'),
('RAP 2', 'CGV Nguyễn Hồng Đào, tp.hcm'),
('RAP 3', 'CGV Aeon Binh Tân,tp.hcm'),
('RAP 4', 'CGV Aeon Tân Phú,tp.hcm'),
('RAP 5', 'CGV Sư Vạn Hạnh,tp.hcm'),
('RAP 6', 'CGV Vincom Center Landmark 81,tp.hcm'),
('RAP 7', 'CGV Hùng Vương Plaza,tp.hcm'),
('RAP 8', 'CGV Pearl Plaza,tp.hcm'),
('RAP 9', 'CGV Lý Chính Thắng,tp.hcm');

create table  phong(
	id int not null auto_increment,
    id_rap int,
    tenphong nvarchar(255),
    primary key(id),
	foreign key(id_rap) references rap(id)
);
INSERT INTO phong (id_rap, tenphong) VALUES 
(1, 'ROOM 1'),
(2, 'ROOM 2'),
(3, 'ROOM 3'),
(4, 'ROOM 4'),
(5, 'ROOM 5'),
(6, 'ROOM 6'),
(7, 'ROOM 7'),
(8, 'ROOM 8'),
(9, 'ROOM 9');

create table  datve(
	id int not null auto_increment,
	id_makhachhang int,
	id_lichchieuphim int, 
    id_phongchuaghe int,
    gia double,
    trangthai_ghe int,
	primary key(id),
	foreign key(id_makhachhang) references khachhang(id),
    foreign key(id_lichchieuphim) references lichchiephim(id),
    foreign key(id_phongchuaghe) references phongchuaghe(id)
    
);
INSERT INTO datve (id_makhachhang, id_lichchieu, id_ghengoi, gia, trangthai_ghe) VALUES
(1, 1, 4, 750000, 1),
(2, 2, 2, 750000, 0),
 (3, 4, 5, 75000, 1),
(4, 6, 6, 90000, 0),
(5, 5, 3, 85000, 0),
(6, 7, 1, 105000, 1);

create table  ghengoi(
	id int not null auto_increment,
    loaighe nchar(20),
    id_phong int,
    hang nchar(10),
    soghe nchar(20),
    primary key(id),
    foreign key(id_phong) references phong(id)
);
INSERT INTO ghengoi (loaighe, id_phong, hang, soghe) VALUES
('Thường', 1, 'A', 1),
('Thường', 1, 'A', 2),
('Thường', 1, 'A', 3),
('Thường', 1, 'A', 4),
('Thường', 1, 'A', 5),
('Thường', 1, 'A', 6),
('Thường', 1, 'A', 7),
('Thường', 1, 'A', 8),
('Thường', 1, 'A', 9),
('Thường', 1, 'B', 1),
('Thường', 1, 'B', 2),
('Thường', 1, 'B', 3),
('Thường', 1, 'B', 4),
('Thường', 1, 'B', 5),
('Thường', 1, 'B', 6),
('Thường', 1, 'B', 7),
('Thường', 1, 'B', 8),
('Thường', 1, 'B', 9),
('Thường', 1, 'C', 1),
('Thường', 1, 'C', 2),
('Thường', 1, 'C', 3),
('Thường', 1, 'C', 4),
('Thường', 1, 'C', 5),
('Thường', 1, 'C', 6),
('Thường', 1, 'C', 7),
('Thường', 1, 'C', 8),
('Thường', 1, 'C', 9),
('Thường', 1, 'D', 1),
('Thường', 1, 'D', 2),
('Thường', 1, 'D', 3),
('Thường', 1, 'D', 4),
('Thường', 1, 'D', 5),
('Thường', 1, 'D', 6),
('Thường', 1, 'D', 7),
('Thường', 1, 'D', 8),
('Thường', 1, 'D', 9),
('VIP', 1, 'E', 1),
('VIP', 1, 'E', 2),
('VIP', 1, 'E', 3),
('VIP', 1, 'E', 4),
('VIP', 1, 'E', 5),
('VIP', 1, 'E', 6),
('VIP', 1, 'E', 7),
('VIP', 1, 'E', 8),
('VIP', 1, 'E', 9),
('VIP', 1, 'F', 1),
('VIP', 1, 'F', 2),
('VIP', 1, 'F', 3),
('VIP', 1, 'F', 4),
('VIP', 1, 'F', 5),
('VIP', 1, 'F', 6),
('VIP', 1, 'F', 7),
('VIP', 1, 'F', 8),
('VIP', 1, 'F', 9),
('Couple', 1, 'G', 1),
('Couple', 1, 'G', 2),
('Couple', 1, 'G', 3),
('Couple', 1, 'G', 4),
('Couple', 1, 'G', 5),
('Couple', 1, 'G', 6),
('Couple', 1, 'G', 7),
('Couple', 1, 'G', 8),
('Couple', 1, 'G', 9),
('Couple', 1, 'H', 1),
('Couple', 1, 'H', 2),
('Couple', 1, 'H', 3),
('Couple', 1, 'H', 4),
('Couple', 1, 'H', 5),
('Couple', 1, 'H', 6),
('Couple', 1, 'H', 7),
('Couple', 1, 'H', 8),
('Couple', 1, 'H', 9),
('Thường', 2, 'A', 1),
('Thường', 2, 'A', 2),
('Thường', 2, 'A', 3),
('Thường', 2, 'A', 4),
('Thường', 2, 'A', 5),
('Thường', 2, 'A', 6),
('Thường', 2, 'A', 7),
('Thường', 2, 'A', 8),
('Thường', 2, 'A', 9),
('Thường', 2, 'B', 1),
('Thường', 2, 'B', 2),
('Thường', 2, 'B', 3),
('Thường', 2, 'B', 4),
('Thường', 2, 'B', 5),
('Thường', 2, 'B', 6),
('Thường', 2, 'B', 7),
('Thường', 2, 'B', 8),
('Thường', 2, 'B', 9),
('Thường', 2, 'C', 1),
('Thường', 2, 'C', 2),
('Thường', 2, 'C', 3),
('Thường', 2, 'C', 4),
('Thường', 2, 'C', 5),
('Thường', 2, 'C', 6),
('Thường', 2, 'C', 7),
('Thường', 2, 'C', 8),
('Thường', 2, 'C', 9),
('Thường', 2, 'D', 1),
('Thường', 2, 'D', 2),
('Thường', 2, 'D', 3),
('Thường', 2, 'D', 4),
('Thường', 2, 'D', 5),
('Thường', 2, 'D', 6),
('Thường', 2, 'D', 7),
('Thường', 2, 'D', 8),
('Thường', 2, 'D', 9),
('VIP', 2, 'E', 1),
('VIP', 2, 'E', 2),
('VIP', 2, 'E', 3),
('VIP', 2, 'E', 4),
('VIP', 2, 'E', 5),
('VIP', 2, 'E', 6),
('VIP', 2, 'E', 7),
('VIP', 2, 'E', 8),
('VIP', 2, 'E', 9),
('VIP', 2, 'F', 1),
('VIP', 2, 'F', 2),
('VIP', 2, 'F', 3),
('VIP', 2, 'F', 4),
('VIP', 2, 'F', 5),
('VIP', 2, 'F', 6),
('VIP', 2, 'F', 7),
('VIP', 2, 'F', 8),
('VIP', 2, 'F', 9),
('Couple', 2, 'G', 1),
('Couple', 2, 'G', 2),
('Couple', 2, 'G', 3),
('Couple', 2, 'G', 4),
('Couple', 2, 'G', 5),
('Couple', 2, 'G', 6),
('Couple', 2, 'G', 7),
('Couple', 2, 'G', 8),
('Couple', 2, 'G', 9),
('Couple', 2, 'H', 1),
('Couple', 2, 'H', 2),
('Couple', 2, 'H', 3),
('Couple', 2, 'H', 4),
('Couple', 2, 'H', 5),
('Couple', 2, 'H', 6),
('Couple', 2, 'H', 7),
('Couple', 2, 'H', 8),
('Couple', 2, 'H', 9),
('Thường', 3, 'A', 1),
('Thường', 3, 'A', 2),
('Thường', 3, 'A', 3),
('Thường', 3, 'A', 4),
('Thường', 3, 'A', 5),
('Thường', 3, 'A', 6),
('Thường', 3, 'A', 7),
('Thường', 3, 'A', 8),
('Thường', 3, 'A', 9),
('Thường', 3, 'B', 1),
('Thường', 3, 'B', 2),
('Thường', 3, 'B', 3),
('Thường', 3, 'B', 4),
('Thường', 3, 'B', 5),
('Thường', 3, 'B', 6),
('Thường', 3, 'B', 7),
('Thường', 3, 'B', 8),
('Thường', 3, 'B', 9),
('Thường', 3, 'C', 1),
('Thường', 3, 'C', 2),
('Thường', 3, 'C', 3),
('Thường', 3, 'C', 4),
('Thường', 3, 'C', 5),
('Thường', 3, 'C', 6),
('Thường', 3, 'C', 7),
('Thường', 3, 'C', 8),
('Thường', 3, 'C', 9),
('Thường', 3, 'D', 1),
('Thường', 3, 'D', 2),
('Thường', 3, 'D', 3),
('Thường', 3, 'D', 4),
('Thường', 3, 'D', 5),
('Thường', 3, 'D', 6),
('Thường', 3, 'D', 7),
('Thường', 3, 'D', 8),
('Thường', 3, 'D', 9),
('VIP', 3, 'E', 1),
('VIP', 3, 'E', 2),
('VIP', 3, 'E', 3),
('VIP', 3, 'E', 4),
('VIP', 3, 'E', 5),
('VIP', 3, 'E', 6),
('VIP', 3, 'E', 7),
('VIP', 3, 'E', 8),
('VIP', 3, 'E', 9),
('VIP', 3, 'F', 1),
('VIP', 3, 'F', 2),
('VIP', 3, 'F', 3),
('VIP', 3, 'F', 4),
('VIP', 3, 'F', 5),
('VIP', 3, 'F', 6),
('VIP', 3, 'F', 7),
('VIP', 3, 'F', 8),
('VIP', 3, 'F', 9),
('Couple', 3, 'G', 1),
('Couple', 3, 'G', 2),
('Couple', 3, 'G', 3),
('Couple', 3, 'G', 4),
('Couple', 3, 'G', 5),
('Couple', 3, 'G', 6),
('Couple', 3, 'G', 7),
('Couple', 3, 'G', 8),
('Couple', 3, 'G', 9),
('Couple', 3, 'H', 1),
('Couple', 3, 'H', 2),
('Couple', 3, 'H', 3),
('Couple', 3, 'H', 4),
('Couple', 3, 'H', 5),
('Couple', 3, 'H', 6),
('Couple', 3, 'H', 7),
('Couple', 3, 'H', 8),
('Couple', 3, 'H', 9),
('Thường', 4, 'A', 1),
('Thường', 4, 'A', 2),
('Thường', 4, 'A', 3),
('Thường', 4, 'A', 4),
('Thường', 4, 'A', 5),
('Thường', 4, 'A', 6),
('Thường', 4, 'A', 7),
('Thường', 4, 'A', 8),
('Thường', 4, 'A', 9),
('Thường', 4, 'B', 1),
('Thường', 4, 'B', 2),
('Thường', 4, 'B', 3),
('Thường', 4, 'B', 4),
('Thường', 4, 'B', 5),
('Thường', 4, 'B', 6),
('Thường', 4, 'B', 7),
('Thường', 4, 'B', 8),
('Thường', 4, 'B', 9),
('Thường', 4, 'C', 1),
('Thường', 4, 'C', 2),
('Thường', 4, 'C', 3),
('Thường', 4, 'C', 4),
('Thường', 4, 'C', 5),
('Thường', 4, 'C', 6),
('Thường', 4, 'C', 7),
('Thường', 4, 'C', 8),
('Thường', 4, 'C', 9),
('Thường', 4, 'D', 1),
('Thường', 4, 'D', 2),
('Thường', 4, 'D', 3),
('Thường', 4, 'D', 4),
('Thường', 4, 'D', 5),
('Thường', 4, 'D', 6),
('Thường', 4, 'D', 7),
('Thường', 4, 'D', 8),
('Thường', 4, 'D', 9),
('VIP', 4, 'E', 1),
('VIP', 4, 'E', 2),
('VIP', 4, 'E', 3),
('VIP', 4, 'E', 4),
('VIP', 4, 'E', 5),
('VIP', 4, 'E', 6),
('VIP', 4, 'E', 7),
('VIP', 4, 'E', 8),
('VIP', 4, 'E', 9),
('VIP', 4, 'F', 1),
('VIP', 4, 'F', 2),
('VIP', 4, 'F', 3),
('VIP', 4, 'F', 4),
('VIP', 4, 'F', 5),
('VIP', 4, 'F', 6),
('VIP', 4, 'F', 7),
('VIP', 4, 'F', 8),
('VIP', 4, 'F', 9),
('Couple', 4, 'G', 1),
('Couple', 4, 'G', 2),
('Couple', 4, 'G', 3),
('Couple', 4, 'G', 4),
('Couple', 4, 'G', 5),
('Couple', 4, 'G', 6),
('Couple', 4, 'G', 7),
('Couple', 4, 'G', 8),
('Couple', 4, 'G', 9),
('Couple', 4, 'H', 1),
('Couple', 4, 'H', 2),
('Couple', 4, 'H', 3),
('Couple', 4, 'H', 4),
('Couple', 4, 'H', 5),
('Couple', 4, 'H', 6),
('Couple', 4, 'H', 7),
('Couple', 4, 'H', 8),
('Couple', 4, 'H', 9),
('Thường', 5, 'A', 1),
('Thường', 5, 'A', 2),
('Thường', 5, 'A', 3),
('Thường', 5, 'A', 4),
('Thường', 5, 'A', 5),
('Thường', 5, 'A', 6),
('Thường', 5, 'A', 7),
('Thường', 5, 'A', 8),
('Thường', 5, 'A', 9),
('Thường', 5, 'B', 1),
('Thường', 5, 'B', 2),
('Thường', 5, 'B', 3),
('Thường', 5, 'B', 4),
('Thường', 5, 'B', 5),
('Thường', 5, 'B', 6),
('Thường', 5, 'B', 7),
('Thường', 5, 'B', 8),
('Thường', 5, 'B', 9),
('Thường', 5, 'C', 1),
('Thường', 5, 'C', 2),
('Thường', 5, 'C', 3),
('Thường', 5, 'C', 4),
('Thường', 5, 'C', 5),
('Thường', 5, 'C', 6),
('Thường', 5, 'C', 7),
('Thường', 5, 'C', 8),
('Thường', 5, 'C', 9),
('Thường', 5, 'D', 1),
('Thường', 5, 'D', 2),
('Thường', 5, 'D', 3),
('Thường', 5, 'D', 4),
('Thường', 5, 'D', 5),
('Thường', 5, 'D', 6),
('Thường', 5, 'D', 7),
('Thường', 5, 'D', 8),
('Thường', 5, 'D', 9),
('VIP', 5, 'E', 1),
('VIP', 5, 'E', 2),
('VIP', 5, 'E', 3),
('VIP', 5, 'E', 4),
('VIP', 5, 'E', 5),
('VIP', 5, 'E', 6),
('VIP', 5, 'E', 7),
('VIP', 5, 'E', 8),
('VIP', 5, 'E', 9),
('VIP', 5, 'F', 1),
('VIP', 5, 'F', 2),
('VIP', 5, 'F', 3),
('VIP', 5, 'F', 4),
('VIP', 5, 'F', 5),
('VIP', 5, 'F', 6),
('VIP', 5, 'F', 7),
('VIP', 5, 'F', 8),
('VIP', 5, 'F', 9),
('Couple', 5, 'G', 1),
('Couple', 5, 'G', 2),
('Couple', 5, 'G', 3),
('Couple', 5, 'G', 4),
('Couple', 5, 'G', 5),
('Couple', 5, 'G', 6),
('Couple', 5, 'G', 7),
('Couple', 5, 'G', 8),
('Couple', 5, 'G', 9),
('Couple', 5, 'H', 1),
('Couple', 5, 'H', 2),
('Couple', 5, 'H', 3),
('Couple', 5, 'H', 4),
('Couple', 5, 'H', 5),
('Couple', 5, 'H', 6),
('Couple', 5, 'H', 7),
('Couple', 5, 'H', 8),
('Couple', 5, 'H', 9),
('Thường', 6, 'A', 1),
('Thường', 6, 'A', 2),
('Thường', 6, 'A', 3),
('Thường', 6, 'A', 4),
('Thường', 6, 'A', 5),
('Thường', 6, 'A', 6),
('Thường', 6, 'A', 7),
('Thường', 6, 'A', 8),
('Thường', 6, 'A', 9),
('Thường', 6, 'B', 1),
('Thường', 6, 'B', 2),
('Thường', 6, 'B', 3),
('Thường', 6, 'B', 4),
('Thường', 6, 'B', 5),
('Thường', 6, 'B', 6),
('Thường', 6, 'B', 7),
('Thường', 6, 'B', 8),
('Thường', 6, 'B', 9),
('Thường', 6, 'C', 1),
('Thường', 6, 'C', 2),
('Thường', 6, 'C', 3),
('Thường', 6, 'C', 4),
('Thường', 6, 'C', 5),
('Thường', 6, 'C', 6),
('Thường', 6, 'C', 7),
('Thường', 6, 'C', 8),
('Thường', 6, 'C', 9),
('Thường', 6, 'D', 1),
('Thường', 6, 'D', 2),
('Thường', 6, 'D', 3),
('Thường', 6, 'D', 4),
('Thường', 6, 'D', 5),
('Thường', 6, 'D', 6),
('Thường', 6, 'D', 7),
('Thường', 6, 'D', 8),
('Thường', 6, 'D', 9),
('VIP', 6, 'E', 1),
('VIP', 6, 'E', 2),
('VIP', 6, 'E', 3),
('VIP', 6, 'E', 4),
('VIP', 6, 'E', 5),
('VIP', 6, 'E', 6),
('VIP', 6, 'E', 7),
('VIP', 6, 'E', 8),
('VIP', 6, 'E', 9),
('VIP', 6, 'F', 1),
('VIP', 6, 'F', 2),
('VIP', 6, 'F', 3),
('VIP', 6, 'F', 4),
('VIP', 6, 'F', 5),
('VIP', 6, 'F', 6),
('VIP', 6, 'F', 7),
('VIP', 6, 'F', 8),
('VIP', 6, 'F', 9),
('Couple', 6, 'G', 1),
('Couple', 6, 'G', 2),
('Couple', 6, 'G', 3),
('Couple', 6, 'G', 4),
('Couple', 6, 'G', 5),
('Couple', 6, 'G', 6),
('Couple', 6, 'G', 7),
('Couple', 6, 'G', 8),
('Couple', 6, 'G', 9),
('Couple', 6, 'H', 1),
('Couple', 6, 'H', 2),
('Couple', 6, 'H', 3),
('Couple', 6, 'H', 4),
('Couple', 6, 'H', 5),
('Couple', 6, 'H', 6),
('Couple', 6, 'H', 7),
('Couple', 6, 'H', 8),
('Couple', 6, 'H', 9),
('Thường', 7, 'A', 1),
('Thường', 7, 'A', 2),
('Thường', 7, 'A', 3),
('Thường', 7, 'A', 4),
('Thường', 7, 'A', 5),
('Thường', 7, 'A', 6),
('Thường', 7, 'A', 7),
('Thường', 7, 'A', 8),
('Thường', 7, 'A', 9),
('Thường', 7, 'B', 1),
('Thường', 7, 'B', 2),
('Thường', 7, 'B', 3),
('Thường', 7, 'B', 4),
('Thường', 7, 'B', 5),
('Thường', 7, 'B', 6),
('Thường', 7, 'B', 7),
('Thường', 7, 'B', 8),
('Thường', 7, 'B', 9),
('Thường', 7, 'C', 1),
('Thường', 7, 'C', 2),
('Thường', 7, 'C', 3),
('Thường', 7, 'C', 4),
('Thường', 7, 'C', 5),
('Thường', 7, 'C', 6),
('Thường', 7, 'C', 7),
('Thường', 7, 'C', 8),
('Thường', 7, 'C', 9),
('Thường', 7, 'D', 1),
('Thường', 7, 'D', 2),
('Thường', 7, 'D', 3),
('Thường', 7, 'D', 4),
('Thường', 7, 'D', 5),
('Thường', 7, 'D', 6),
('Thường', 7, 'D', 7),
('Thường', 7, 'D', 8),
('Thường', 7, 'D', 9),
('VIP', 7, 'E', 1),
('VIP', 7, 'E', 2),
('VIP', 7, 'E', 3),
('VIP', 7, 'E', 4),
('VIP', 7, 'E', 5),
('VIP', 7, 'E', 6),
('VIP', 7, 'E', 7),
('VIP', 7, 'E', 8),
('VIP', 7, 'E', 9),
('VIP', 7, 'F', 1),
('VIP', 7, 'F', 2),
('VIP', 7, 'F', 3),
('VIP', 7, 'F', 4),
('VIP', 7, 'F', 5),
('VIP', 7, 'F', 6),
('VIP', 7, 'F', 7),
('VIP', 7, 'F', 8),
('VIP', 7, 'F', 9),
('Couple', 7, 'G', 1),
('Couple', 7, 'G', 2),
('Couple', 7, 'G', 3),
('Couple', 7, 'G', 4),
('Couple', 7, 'G', 5),
('Couple', 7, 'G', 6),
('Couple', 7, 'G', 7),
('Couple', 7, 'G', 8),
('Couple', 7, 'G', 9),
('Couple', 7, 'H', 1),
('Couple', 7, 'H', 2),
('Couple', 7, 'H', 3),
('Couple', 7, 'H', 4),
('Couple', 7, 'H', 5),
('Couple', 7, 'H', 6),
('Couple', 7, 'H', 7),
('Couple', 7, 'H', 8),
('Couple', 7, 'H', 9),
('Thường', 8, 'A', 1),
('Thường', 8, 'A', 2),
('Thường', 8, 'A', 3),
('Thường', 8, 'A', 4),
('Thường', 8, 'A', 5),
('Thường', 8, 'A', 6),
('Thường', 8, 'A', 7),
('Thường', 8, 'A', 8),
('Thường', 8, 'A', 9),
('Thường', 8, 'B', 1),
('Thường', 8, 'B', 2),
('Thường', 8, 'B', 3),
('Thường', 8, 'B', 4),
('Thường', 8, 'B', 5),
('Thường', 8, 'B', 6),
('Thường', 8, 'B', 7),
('Thường', 8, 'B', 8),
('Thường', 8, 'B', 9),
('Thường', 8, 'C', 1),
('Thường', 8, 'C', 2),
('Thường', 8, 'C', 3),
('Thường', 8, 'C', 4),
('Thường', 8, 'C', 5),
('Thường', 8, 'C', 6),
('Thường', 8, 'C', 7),
('Thường', 8, 'C', 8),
('Thường', 8, 'C', 9),
('Thường', 8, 'D', 1),
('Thường', 8, 'D', 2),
('Thường', 8, 'D', 3),
('Thường', 8, 'D', 4),
('Thường', 8, 'D', 5),
('Thường', 8, 'D', 6),
('Thường', 8, 'D', 7),
('Thường', 8, 'D', 8),
('Thường', 8, 'D', 9),
('VIP', 8, 'E', 1),
('VIP', 8, 'E', 2),
('VIP', 8, 'E', 3),
('VIP', 8, 'E', 4),
('VIP', 8, 'E', 5),
('VIP', 8, 'E', 6),
('VIP', 8, 'E', 7),
('VIP', 8, 'E', 8),
('VIP', 8, 'E', 9),
('VIP', 8, 'F', 1),
('VIP', 8, 'F', 2),
('VIP', 8, 'F', 3),
('VIP', 8, 'F', 4),
('VIP', 8, 'F', 5),
('VIP', 8, 'F', 6),
('VIP', 8, 'F', 7),
('VIP', 8, 'F', 8),
('VIP', 8, 'F', 9),
('Couple', 1, 'G', 1),
('Couple', 8, 'G', 2),
('Couple', 8, 'G', 3),
('Couple', 8, 'G', 4),
('Couple', 8, 'G', 5),
('Couple', 8, 'G', 6),
('Couple', 8, 'G', 7),
('Couple', 8, 'G', 8),
('Couple', 8, 'G', 9),
('Couple', 8, 'H', 1),
('Couple', 8, 'H', 2),
('Couple', 8, 'H', 3),
('Couple', 8, 'H', 4),
('Couple', 8, 'H', 5),
('Couple', 8, 'H', 6),
('Couple', 8, 'H', 7),
('Couple', 8, 'H', 8),
('Couple', 8, 'H', 9),
('Thường', 9, 'A', 1),
('Thường', 9, 'A', 2),
('Thường', 9, 'A', 3),
('Thường', 9, 'A', 4),
('Thường', 9, 'A', 5),
('Thường', 9, 'A', 6),
('Thường', 9, 'A', 7),
('Thường', 9, 'A', 8),
('Thường', 9, 'A', 9),
('Thường', 9, 'B', 1),
('Thường', 9, 'B', 2),
('Thường', 9, 'B', 3),
('Thường', 9, 'B', 4),
('Thường', 9, 'B', 5),
('Thường', 9, 'B', 6),
('Thường', 9, 'B', 7),
('Thường', 9, 'B', 8),
('Thường', 9, 'B', 9),
('Thường', 9, 'C', 1),
('Thường', 9, 'C', 2),
('Thường', 9, 'C', 3),
('Thường', 9, 'C', 4),
('Thường', 9, 'C', 5),
('Thường', 9, 'C', 6),
('Thường', 9, 'C', 7),
('Thường', 9, 'C', 8),
('Thường', 9, 'C', 9),
('Thường', 9, 'D', 1),
('Thường', 9, 'D', 2),
('Thường', 9, 'D', 3),
('Thường', 9, 'D', 4),
('Thường', 9, 'D', 5),
('Thường', 9, 'D', 6),
('Thường', 9, 'D', 7),
('Thường', 9, 'D', 8),
('Thường', 9, 'D', 9),
('VIP', 9, 'E', 1),
('VIP', 9, 'E', 2),
('VIP', 9, 'E', 3),
('VIP', 9, 'E', 4),
('VIP', 9, 'E', 5),
('VIP', 9, 'E', 6),
('VIP', 9, 'E', 7),
('VIP', 9, 'E', 8),
('VIP', 9, 'E', 9),
('VIP', 9, 'F', 1),
('VIP', 9, 'F', 2),
('VIP', 9, 'F', 3),
('VIP', 9, 'F', 4),
('VIP', 9, 'F', 5),
('VIP', 9, 'F', 6),
('VIP', 9, 'F', 7),
('VIP', 9, 'F', 8),
('VIP', 9, 'F', 9),
('Couple', 9, 'G', 1),
('Couple', 9, 'G', 2),
('Couple', 9, 'G', 3),
('Couple', 9, 'G', 4),
('Couple', 9, 'G', 5),
('Couple', 9, 'G', 6),
('Couple', 9, 'G', 7),
('Couple', 9, 'G', 8),
('Couple', 9, 'G', 9),
('Couple', 9, 'H', 1),
('Couple', 9, 'H', 2),
('Couple', 9, 'H', 3),
('Couple', 9, 'H', 4),
('Couple', 9, 'H', 5),
('Couple', 9, 'H', 6),
('Couple', 9, 'H', 7),
('Couple', 9, 'H', 8),
('Couple', 9, 'H', 9);

create table  phongchuaghe(
	id int not null auto_increment,
    id_ghe int,
    id_phong int,
    primary key(id),
    foreign key(id_phong) references phong(id),
	foreign key(id_ghe) references ghengoi(id)
);

INSERT INTO phongchuaghe (id,id_ghe,id_phong) VALUES 
(1,1,2),
(2,2,3),
(3,3,5),
(4,4,4),
(5,5,7),
(6,6,6),
(7,7,8),
(8,8,9),
(9,9,1);



create table phim(
	id int not null auto_increment,
    id_quocgia int,
	tenphim varchar(255),
    mota text , 
    trailerphim text,
    theloai varchar(255),
    dienvien varchar(255),
    nhasanxuat varchar(255),
    ngaykhoichieu datetime,
    thoiluong nchar(20),
    poster text,
    phimcens nchar(20),
	primary key(id),
    foreign key(id_quocgia) references quocgia(id)
	
);
INSERT INTO phim (id,  id_quocgia, tenphim, mota, trailerphim, theloai, dienvien, nhasanxuat, ngaykhoichieu, thoiluong, poster,phimcens) VALUES
(1, 1,'Liên minh siêu thú', 'Trong “Liên Minh Siêu Thú DC”, Siêu Cún Krypto và Superman là cặp bài trùng
không thể tách rời, cùng sở hữu những siêu năng lực tương tự và cùng nhau chiến đấu chống lại tội phạm tại
thành phố Metropolis. Khi Superman và những thành viên của Liên Minh Công Lý bị bắt cóc, Krypto phải thuyết phục 
cậu chàng Ace luộm thuộm, nàng Heo PB, Rùa Merton và Sóc Chip khai phá những sức mạnh tiềm ẩn và cùng nhau giải 
các siêu anh hùng. “Liên Minh Siêu Thú DC” có sự góp giọng của bộ đôi ngôi sao nổi tiếng bậc nhất
Hollywood Dwayne Johnson (lồng tiếng cho Siêu cún Krypto) và Kevin Hart (Superman). Đặc biệt, tài tử Keanu Reeves 
sẽ đảm nhận những câu thoại chất lừ đến từ Batman.', 'https://youtu.be/GrBRGS6Z-jI', 'Hoạt hình',
'wayne Johnson, Kevin Hart, Keanu Reeves', 'DC Entertainment, Seven Bucks Productions, Warner Animation Group',
'2022-07-29', '1:46:00', 'https://www.cgv.vn/default/dc-pets.html', 'P'),
(2, 1,'Chuyện ma giảng đường - học kỳ 2', 'bộ phim dựa trên mạch truyện kinh dị lấy bối cảnh trong
khuôn viên trường đại học, đề cập đến những rắc rối và nỗi bức xúc mà thanh thiếu niên cũng như thế hệ trẻ 
đang trải qua trong quá trình chuyển đổi. Lấy ý tưởng về “những bóng ma trong trường đại học”, bộ phim được 
xây dựng kịch bản từ 3 trong số những câu chuyện ma rùng rợn nhất có thật tại các trường đại học bị ma ám với cách
kể chuyện mới lạ nhưng vẫn duy trì được tất cả các yếu tố kinh dị khiến Haunted Universities Second Semester
(tựa Việt: Chuyện Ma Giảng Đường – Học Kỳ 2) trở nên rùng rợn theo cách riêng, vượt ra khỏi khuôn khổ
của một câu chuyện ma quen thuộc với cách lột tả nội dung chân thực nhất liên quan sâu sắc đến xã hội ngày nay.',
'https://youtu.be/7Q72BKYxYVg', 'Kinh dị', 'Krit Jirapattananuwong, Kemisra Poladej, Panisra Rikulsurakarn,
Teeradon “James” Supanpinyo', 'Pataraporn Weerasakwong , Jatupong Rungruangdechapat, Ekapon Sethasuk', '2022-07-29',
'2:06:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/p/o/poster_700_x_1000_px.jpg',
'c16'),
(3,3, 'Điều ước cuối của tù nhân 2037', ' Ở tuổi 19, thay vì đến trường như bao bạn bè đồng trang lứa,
Yoon-yeong phải nỗ lực hết mình để đi làm thêm kiếm tiền. Khao khát một cuộc sống tốt đẹp hơn cho mình và người mẹ
khiếm thính, Yoon-yeong đặt mục tiêu thi đỗ kỳ thi công chức lên trên hết. Bất ngờ và trớ trêu thay,
một sự cố khủng khiếp xảy ra, biến Yoon-yeong từ nạn nhân đáng thương trở thành kẻ giết người.
Trong thời điểm tuyệt vọng và bất lực nhất, Yoon-yeong đã gặp những người chị em trong phòng giam số 12.
Đằng sau mỗi người là một câu chuyện khác nhau, nhưng họ đã trao nhau tình yêu thương và niềm hy vọng để cùng
hướng về một tương lai tươi sáng ngoài song sắt nhà tù.', 'https://youtu.be/1AXeEfyTLKs', 'Tâm lý', 'Hong Je Yi, 
Kim Ji Young, Kim Mi Hwa, Hwang Seok Jeong, Shin Eun Hung, Jeon So Min, Yoon Mi Kyung, Jung In Ki', 'Mo Hong Jin',
'2022-07-29', '1:51:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/m/g/mg_main-poster_layered_1_.jpg',
'C16'),
(4,2, 'Dân chơi không sợ con rơi', 'Quân, một dân chơi miệt vườn với tài “sát gái” thượng hạng, bỗng một ngày 
phải gánh lấy "cục nợ con rơi" từ bạn gái cũ. Được sự trợ giúp "ba phần đắc lực bảy phần phá đám" của Tám Mánh,
Quân dần mất hết hy vọng tìm lại mẹ cho đứa bé và dần trở thành người cha hoàn hảo trong mắt Bé Thỏ. Cuộc sống đầy 
sắc màu và tiếng cười của hai cha con bị đặt trước thử thách lớn lao khi người mẹ mà Bé Thỏ ngày đêm mong ngóng
cuối cùng cũng xuất hiện…', 'https://youtu.be/34OJjBVw3do', 'Hài, Tâm lý', 'Tiến Luật, bé Bảo Thi, Huỳnh Phương, Nguyên Thảo, Vân Trang,
Ngô Kiến Huy, Lê Khánh,...', 'Huỳnh Đông', '2022-07-29', '1:28:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/8/0/800x1200-dckscr_1_.jpg',
'C13'),
(5, 09,3, 'Quyết tâm chia tay', 'Một thanh tra cảnh sát rơi vào lưới tình với goá phụ trẻ - nghi phạm giết người trong vụ án mà anh ta đang điều tra.',
'https://youtu.be/kdJvKwT2NAU', 'Hồi hộp, Tình cảm', 'Tang Wei, Park Hae Il', 'Park Chan Wook', '2022-07-15', '2:18:00',
'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/d/t/dtl_m-poster_w700xh1000_1_.jpg',
'C16'),
(6, 1, 'MINIONS: SỰ TRỖI DẬY CỦA GRU', 'Rất lâu trước khi trở thành một phản diện chuyên nghiệp, Gru chỉ là một
cậu bé 12 tuổi sống ở vùng ngoại ô vào những năm 1970, với ước mơ một ngày sẽ làm “bá chủ” thế giới. Và 3
“quả chuối” vàng biết đi từ trên trời rơi xuống đã là những đồng đội đầu tiên của Gru, bên cạnh ủng hộ cậu bé, 
cùng nhau thiết kế những vũ khí đầu tiên, thực hiện những phi vụ đầu tiên.', 'https://youtu.be/dTQXlDV16SY', 
'Hài, hoạt hình, phiêu lưu', 'Steve Carell, Lucy Lawless, Michelle Yeoh...', 'Kyle Balda, Brad Ableson, Jonathan del Val',
'2022-07-01', '1:28:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/m/i/minions_2_-_kc_01.7.22_1_.jpg',
'p'),
(7,  1,'Linh hồn vũ nữ', 'Sáu học sinh đi vào một ngôi làng hẻo lánh để thực hiện chương trình công ích. Và họ không biết được rằng đây là vùng đất của những
linh hồn bị mắc kẹt dưới sự cai trị của Badarawuhi – một linh hồn vũ nữ', 'https://youtu.be/-Q1kXe1X0js', 'Kinh dị', 
'Aghniny Haque, Achmad Megantara, Aulia Sarah, Adinda Thomas, Tissa Biani, Fajar Nugra, Calvin Jeremy, v.v', 'Awi Suryadi',
'2022-07-22', '2:03:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/l/i/linh_h_n_v_n_-_payoff_poster_k_ch_th_c_facebook_-_kc_22.07.2022_1_.jpg',
'C16'),
(8,  2,'Là mây trên bầu trời ai đó', 'Nàng tiểu thư đỏng đảnh Mây sang Thái Lan để theo đuổi một “nam thần” nổi tiếng, tình cờ lại va phải anh chàng “oan gia” và 
cùng nhau trải qua những tình huống dở khóc dở cười', 'https://youtu.be/U4OwkJ9NLWw', 'Hài, tình cảm', 
'Ngọc Lan Vy, August Vachiravit, Hồng Vân, Trịnh Tú Trung, Hải Triều, Lưu Đông, Lâm Bảo Châu, Quỳnh Lý, Nhâm Phương Nam, Puvadol Vechwongsa, Pakorn Lum và Push Puttichai.',
'Thanadit Pradit', '2022-07-27', '1:29:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/l/a/lamay_teaser_1_.jpg', 
'C13'),
(9,  1,'PHI CÔNG SIÊU ĐẲNG MAVERICK', 'Sau hơn ba mươi năm phục vụ, Pete “Maverick” Mitchell từng nổi danh
là một phi công thử nghiệm quả cảm hàng đầu của Hải quân, né tránh cơ hội thăng chức, điều khiến anh cảm thấy 
bị bó buộc, để trở về làm chính mình.', 'https://youtu.be/ZR99nOkEolM', 'Hành động, phiêu lưu', 'Tom Cruise, Justin Marks, Peter Craig, Eric Warren Singer',
'Joseph Kosinski', '2022-05-27', '2:10:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/t/o/top_gun_maverick_-_poster_28.03_1_.jpg',
'C13'),
(10,  3,'THOÁT KHỎI MOGADISHU', 'Dựa trên một sự kiện lịch sử khó tin nhưng có thật, bộ phim tái hiện lại lần hợp tác gay cấn, chưa từng có tiền lệ giữa các
nhà ngoại giao Nam và Bắc Hàn nhằm giải thoát công dân của họ khỏi cuộc binh biến tại Somalia vào năm 1991.', 
'https://youtu.be/yz5XSicbWvQ', 'Hành động, hồi hộp, tâm lý', 'Kim Yoon-Seok, Jo In-Sung, Huh Joon-Ho,...', 
'Ryoo Seung-wan', '2022-07-15', '2:01:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/m/a/main_poster_tho_t_kh_i_mogadishu_1_.jpg',
'C16');

create table  lichchieu(
	id int not null auto_increment,
    lichchieu date,
    thoigianbatdau time,
	thoigianketthuc time,
	primary key(id)
    
    
);
INSERT INTO lichchieu (lichchieu, thoigianbatdau, thoigianketthuc) VALUES 
( '2019-12-10', '17:15:00', '19:00:00'),
( '2019-12-11', '13:00:00', '14:30:00'),
( '2019-12-10', '17:15:00', '19:00:00'),
( '2019-12-12', '17:15:00', '19:00:00'),
( '2019-12-10', '17:15:00', '19:00:00'),
( '2019-12-11', '17:15:00', '19:00:00'),
( '2019-12-08', '17:15:00', '19:00:00'),
( '2019-12-16', '17:15:00', '19:00:00'),
( '2019-12-10', '14:15:00', '16:00:00');

create database QLDatVeXemPhim;
use  QLDatVeXemPhim ;

DROP DATABASE QLDatVeXemPhim;


create table quocgia(
	id int auto_increment,
    ten_quocgia varchar(255),
    primary key(id)
);
INSERT INTO quocgia (id,ten_quocgia) VALUES
(1,'AMERICA'),
(2,'VIETNAM'),
(3,'KOREA'),
(4,'JAPAN'),
(5,'FRANCE'),
(6,'ENGLAND');
create table  khachhang(
	id int not null auto_increment,
    hoten nvarchar(255),
    matkhau nchar(10),
    taikhoan nchar(20),
    hinhdaidien text,
    email nvarchar(255),
    diachi nvarchar(255),
    dienthoai  nchar(10),
    gioitinh nchar(10),
    ngaysinh DATE ,
    primary key(id)
);
INSERT INTO khachhang (hoten, matkhau, taikhoan, hinhdaidien, email, diachi, dienthoai, gioitinh, ngaysinh) VALUES
('Nguyễn Văn Trung', '123456', 'nguyenvantrung', '1.jpg', 'nguyenvantrung@gmail.com', '525/7/14 lê trọng tấn, Tp.HCM', '0123456781', 'NAM', '1997-05-11') ,
('Trần Công Tùng', '123456', 'trancongtung', '2.jpg', 'trancongtung@gmail.com', '5 Lê Niệm, Tp.HCM', '0123456782', 'NAM', '1997-01-04'),
('Lê Ngọc Ánh', '123456', 'lengocanh', '3.jpg', 'lengocanh@gmail.com', '41 Trường Chinh, Tp.HCM', '0123456783', 'NỮ', '1981-09-12'),
('Nguyễn Giang Chương', '123456', 'nguyengiangchuong', '4.jpg', 'nguyengiangchuong@gmail.com', '400 Phan Văn Trị, Tp.HCM', '0123456784', 'NAM', '1993-01-03'),
('Nguyễn Thị Bình', '123456', 'nguyenthibinh', '5.jpg', 'nguyenthibinh@gmail.com', '20 Nguyễn Trãi, Tp.HCM', '0123456785', 'NỮ', '1972-12-05'),
('Phạm Trung Kiên', '123456', 'phamtrungkien', '6.jpg', 'phamtrungkien@gmail.com', '11 Lê Văn Quới, Tp.HCM', '0123456786', 'NAM', '2000-05-18'),
('Ngô Thị Kim Nhung', '123456', 'ngothikimnhung', '7.jpg', 'ngothikimnhung@gmail.com', '77/10 Nguyễn Thị Minh Khai, Tp.HCM', '0123456787', 'NỮ', '1994-01-10'),
('Nguyễn Minh Đức', '123456', 'nguyenminhduc', '8.jpg', 'nguyenminhduc@gmail.com', '146/19 Võ Thị Sáu, Tp.HCM', '0123456788', 'NAM', '2002-01-07'),
('Hạ Quang Minh', '123456', 'haquangminh', '9.jpg', 'haquangminh@gmail.com', '128/2 Nguyễn Thị Định, Tp.HCM', '0123456789', 'NAM', '1984-09-06'),
('Thanh Hanh', '123456', 'thanhhanh', '10.jpg', 'thanhhanh@gmail.com', '215 Thành Thái, Tp.HCM', '0123456780', 'NỮ', '1997-05-05') ;

create table roleskh(
   id_role int,
   tenrole varchar(25),
   primary key(id_role)
);
INSERT INTO roleskh (id_role, tenrole) VALUES
(1,'ROLE_USER'),
(2,'ROLE_ADMIN');

create table role_khachhang(
	role_id int,
    user_id int,
    primary key(role_id,user_id),
    foreign key(role_id) references roleskh(id_role),
    foreign key(user_id) references khachhang(id)
);
INSERT INTO role_khachhang(role_id, user_id) VALUES
(1,1),
(2,2),
(1,3),
(2,4),
(1,5),
(1,6),
(2,7),
(1,8),
(1,9),
(2,10);

create table  rap(
	id int not null auto_increment,
    tenrap nvarchar(255),
    diachi nvarchar(255),
    primary key(id)
    
);
INSERT INTO rap (tenrap, diachi) VALUES 
('RAP 1', 'CGV vicom Nguyễn Thị Nghĩa,tp.hcm'),
('RAP 2', 'CGV Nguyễn Hồng Đào, tp.hcm'),
('RAP 3', 'CGV Aeon Binh Tân,tp.hcm'),
('RAP 4', 'CGV Aeon Tân Phú,tp.hcm'),
('RAP 5', 'CGV Sư Vạn Hạnh,tp.hcm'),
('RAP 6', 'CGV Vincom Center Landmark 81,tp.hcm'),
('RAP 7', 'CGV Hùng Vương Plaza,tp.hcm'),
('RAP 8', 'CGV Pearl Plaza,tp.hcm'),
('RAP 9', 'CGV Lý Chính Thắng,tp.hcm');

create table  phong(
	id int not null auto_increment,
    id_rap int,
    tenphong nvarchar(255),
    primary key(id),
	foreign key(id_rap) references rap(id)
);
INSERT INTO phong (id_rap, tenphong) VALUES 
(1, 'ROOM 1'),
(2, 'ROOM 2'),
(3, 'ROOM 3'),
(4, 'ROOM 4'),
(5, 'ROOM 5'),
(6, 'ROOM 6'),
(7, 'ROOM 7'),
(8, 'ROOM 8'),
(9, 'ROOM 9');

create table  lichchieu(
	id int not null auto_increment,
    lichchieu date,
    thoigianbatdau time,
	thoigianketthuc time,
	primary key(id)
    
    
);

INSERT INTO lichchieu (lichchieu, thoigianbatdau, thoigianketthuc) VALUES 
( '2019-12-10', '17:15:00', '19:00:00'),
( '2019-12-11', '13:00:00', '14:30:00'),
( '2019-12-10', '17:15:00', '19:00:00'),
( '2019-12-12', '17:15:00', '19:00:00'),
( '2019-12-10', '17:15:00', '19:00:00'),
( '2019-12-11', '17:15:00', '19:00:00'),
( '2019-12-08', '17:15:00', '19:00:00'),
( '2019-12-16', '17:15:00', '19:00:00'),
( '2019-12-10', '14:15:00', '16:00:00');

create table phim(
	id int not null auto_increment,
    id_quocgia int,
	tenphim varchar(255),
    mota text , 
    trailerphim text,
    theloai varchar(255),
    dienvien varchar(255),
    nhasanxuat varchar(255),
    ngaykhoichieu datetime,
    thoiluong nchar(20),
    poster text,
    phimcens nchar(20),
	primary key(id),
    foreign key(id_quocgia) references quocgia(id)
	
);
INSERT INTO phim (id,id_quocgia, tenphim, mota, trailerphim, theloai, dienvien, nhasanxuat, ngaykhoichieu, thoiluong, poster,phimcens) VALUES
(1, 1,'Liên minh siêu thú', 'Trong “Liên Minh Siêu Thú DC”, Siêu Cún Krypto và Superman là cặp bài trùng
không thể tách rời, cùng sở hữu những siêu năng lực tương tự và cùng nhau chiến đấu chống lại tội phạm tại
thành phố Metropolis. Khi Superman và những thành viên của Liên Minh Công Lý bị bắt cóc, Krypto phải thuyết phục 
cậu chàng Ace luộm thuộm, nàng Heo PB, Rùa Merton và Sóc Chip khai phá những sức mạnh tiềm ẩn và cùng nhau giải 
các siêu anh hùng. “Liên Minh Siêu Thú DC” có sự góp giọng của bộ đôi ngôi sao nổi tiếng bậc nhất
Hollywood Dwayne Johnson (lồng tiếng cho Siêu cún Krypto) và Kevin Hart (Superman). Đặc biệt, tài tử Keanu Reeves 
sẽ đảm nhận những câu thoại chất lừ đến từ Batman.', 'https://youtu.be/GrBRGS6Z-jI', 'Hoạt hình',
'wayne Johnson, Kevin Hart, Keanu Reeves', 'DC Entertainment, Seven Bucks Productions, Warner Animation Group',
'2022-07-29', '1:46:00', 'https://www.cgv.vn/default/dc-pets.html', 'P'),
(2, 1,'Chuyện ma giảng đường - học kỳ 2', 'bộ phim dựa trên mạch truyện kinh dị lấy bối cảnh trong
khuôn viên trường đại học, đề cập đến những rắc rối và nỗi bức xúc mà thanh thiếu niên cũng như thế hệ trẻ 
đang trải qua trong quá trình chuyển đổi. Lấy ý tưởng về “những bóng ma trong trường đại học”, bộ phim được 
xây dựng kịch bản từ 3 trong số những câu chuyện ma rùng rợn nhất có thật tại các trường đại học bị ma ám với cách
kể chuyện mới lạ nhưng vẫn duy trì được tất cả các yếu tố kinh dị khiến Haunted Universities Second Semester
(tựa Việt: Chuyện Ma Giảng Đường – Học Kỳ 2) trở nên rùng rợn theo cách riêng, vượt ra khỏi khuôn khổ
của một câu chuyện ma quen thuộc với cách lột tả nội dung chân thực nhất liên quan sâu sắc đến xã hội ngày nay.',
'https://youtu.be/7Q72BKYxYVg', 'Kinh dị', 'Krit Jirapattananuwong, Kemisra Poladej, Panisra Rikulsurakarn,
Teeradon “James” Supanpinyo', 'Pataraporn Weerasakwong , Jatupong Rungruangdechapat, Ekapon Sethasuk', '2022-07-29',
'2:06:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/p/o/poster_700_x_1000_px.jpg',
'c16'),
(3,3, 'Điều ước cuối của tù nhân 2037', ' Ở tuổi 19, thay vì đến trường như bao bạn bè đồng trang lứa,
Yoon-yeong phải nỗ lực hết mình để đi làm thêm kiếm tiền. Khao khát một cuộc sống tốt đẹp hơn cho mình và người mẹ
khiếm thính, Yoon-yeong đặt mục tiêu thi đỗ kỳ thi công chức lên trên hết. Bất ngờ và trớ trêu thay,
một sự cố khủng khiếp xảy ra, biến Yoon-yeong từ nạn nhân đáng thương trở thành kẻ giết người.
Trong thời điểm tuyệt vọng và bất lực nhất, Yoon-yeong đã gặp những người chị em trong phòng giam số 12.
Đằng sau mỗi người là một câu chuyện khác nhau, nhưng họ đã trao nhau tình yêu thương và niềm hy vọng để cùng
hướng về một tương lai tươi sáng ngoài song sắt nhà tù.', 'https://youtu.be/1AXeEfyTLKs', 'Tâm lý', 'Hong Je Yi, 
Kim Ji Young, Kim Mi Hwa, Hwang Seok Jeong, Shin Eun Hung, Jeon So Min, Yoon Mi Kyung, Jung In Ki', 'Mo Hong Jin',
'2022-07-29', '1:51:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/m/g/mg_main-poster_layered_1_.jpg',
'C16'),
(4,2, 'Dân chơi không sợ con rơi', 'Quân, một dân chơi miệt vườn với tài “sát gái” thượng hạng, bỗng một ngày 
phải gánh lấy "cục nợ con rơi" từ bạn gái cũ. Được sự trợ giúp "ba phần đắc lực bảy phần phá đám" của Tám Mánh,
Quân dần mất hết hy vọng tìm lại mẹ cho đứa bé và dần trở thành người cha hoàn hảo trong mắt Bé Thỏ. Cuộc sống đầy 
sắc màu và tiếng cười của hai cha con bị đặt trước thử thách lớn lao khi người mẹ mà Bé Thỏ ngày đêm mong ngóng
cuối cùng cũng xuất hiện…', 'https://youtu.be/34OJjBVw3do', 'Hài, Tâm lý', 'Tiến Luật, bé Bảo Thi, Huỳnh Phương, Nguyên Thảo, Vân Trang,
Ngô Kiến Huy, Lê Khánh,...', 'Huỳnh Đông', '2022-07-29', '1:28:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/8/0/800x1200-dckscr_1_.jpg',
'C13'),
(5, 3, 'Quyết tâm chia tay', 'Một thanh tra cảnh sát rơi vào lưới tình với goá phụ trẻ - nghi phạm giết người trong vụ án mà anh ta đang điều tra.',
'https://youtu.be/kdJvKwT2NAU', 'Hồi hộp, Tình cảm', 'Tang Wei, Park Hae Il', 'Park Chan Wook', '2022-07-15', '2:18:00',
'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/d/t/dtl_m-poster_w700xh1000_1_.jpg',
'C16'),
(6, 1, 'MINIONS: SỰ TRỖI DẬY CỦA GRU', 'Rất lâu trước khi trở thành một phản diện chuyên nghiệp, Gru chỉ là một
cậu bé 12 tuổi sống ở vùng ngoại ô vào những năm 1970, với ước mơ một ngày sẽ làm “bá chủ” thế giới. Và 3
“quả chuối” vàng biết đi từ trên trời rơi xuống đã là những đồng đội đầu tiên của Gru, bên cạnh ủng hộ cậu bé, 
cùng nhau thiết kế những vũ khí đầu tiên, thực hiện những phi vụ đầu tiên.', 'https://youtu.be/dTQXlDV16SY', 
'Hài, hoạt hình, phiêu lưu', 'Steve Carell, Lucy Lawless, Michelle Yeoh...', 'Kyle Balda, Brad Ableson, Jonathan del Val',
'2022-07-01', '1:28:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/m/i/minions_2_-_kc_01.7.22_1_.jpg',
'p'),
(7, 1,'Linh hồn vũ nữ', 'Sáu học sinh đi vào một ngôi làng hẻo lánh để thực hiện chương trình công ích. Và họ không biết được rằng đây là vùng đất của những
linh hồn bị mắc kẹt dưới sự cai trị của Badarawuhi – một linh hồn vũ nữ', 'https://youtu.be/-Q1kXe1X0js', 'Kinh dị', 
'Aghniny Haque, Achmad Megantara, Aulia Sarah, Adinda Thomas, Tissa Biani, Fajar Nugra, Calvin Jeremy, v.v', 'Awi Suryadi',
'2022-07-22', '2:03:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/l/i/linh_h_n_v_n_-_payoff_poster_k_ch_th_c_facebook_-_kc_22.07.2022_1_.jpg',
'C16'),
(8, 2,'Là mây trên bầu trời ai đó', 'Nàng tiểu thư đỏng đảnh Mây sang Thái Lan để theo đuổi một “nam thần” nổi tiếng, tình cờ lại va phải anh chàng “oan gia” và 
cùng nhau trải qua những tình huống dở khóc dở cười', 'https://youtu.be/U4OwkJ9NLWw', 'Hài, tình cảm', 
'Ngọc Lan Vy, August Vachiravit, Hồng Vân, Trịnh Tú Trung, Hải Triều, Lưu Đông, Lâm Bảo Châu, Quỳnh Lý, Nhâm Phương Nam, Puvadol Vechwongsa, Pakorn Lum và Push Puttichai.',
'Thanadit Pradit', '2022-07-27', '1:29:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/l/a/lamay_teaser_1_.jpg', 
'C13'),
(9,  1,'PHI CÔNG SIÊU ĐẲNG MAVERICK', 'Sau hơn ba mươi năm phục vụ, Pete “Maverick” Mitchell từng nổi danh
là một phi công thử nghiệm quả cảm hàng đầu của Hải quân, né tránh cơ hội thăng chức, điều khiến anh cảm thấy 
bị bó buộc, để trở về làm chính mình.', 'https://youtu.be/ZR99nOkEolM', 'Hành động, phiêu lưu', 'Tom Cruise, Justin Marks, Peter Craig, Eric Warren Singer',
'Joseph Kosinski', '2022-05-27', '2:10:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/t/o/top_gun_maverick_-_poster_28.03_1_.jpg',
'C13'),
(10, 3,'THOÁT KHỎI MOGADISHU', 'Dựa trên một sự kiện lịch sử khó tin nhưng có thật, bộ phim tái hiện lại lần hợp tác gay cấn, chưa từng có tiền lệ giữa các
nhà ngoại giao Nam và Bắc Hàn nhằm giải thoát công dân của họ khỏi cuộc binh biến tại Somalia vào năm 1991.', 
'https://youtu.be/yz5XSicbWvQ', 'Hành động, hồi hộp, tâm lý', 'Kim Yoon-Seok, Jo In-Sung, Huh Joon-Ho,...', 
'Ryoo Seung-wan', '2022-07-15', '2:01:00', 'https://www.cgv.vn/media/catalog/product/cache/1/image/c5f0a1eff4c394a251036189ccddaacd/m/a/main_poster_tho_t_kh_i_mogadishu_1_.jpg',
'C16');


create table lichchieupphim(
	id int not null auto_increment,
	id_phim int,
    id_phong int,
    id_lichchieu int,
	primary key(id),
    foreign key(id_phim) references phim(id),
    foreign key(id_phong) references phong(id)
);

INSERT INTO lichchieupphim (id,id_phim,id_phong,id_lichchieu ) VALUES 
(1,1, 1, 9),
(2,2, 2, 1),
(3,1, 5, 2),
(4,1, 8, 3),
(5,3, 3, 4),
(6,5, 6, 5),
(7,4, 9, 7),
(8,2, 5,9 ),
(9,6, 4, 8);

create table  ghengoi(
	id int not null auto_increment,
    loaighe nchar(20),
    id_phong int,
    hang nchar(10),
    soghe nchar(20),
    primary key(id),
    foreign key(id_phong) references phong(id)
);

INSERT INTO ghengoi (loaighe, id_phong, hang, soghe) VALUES
('Thường', 1, 'A', 1),
('Thường', 1, 'A', 2),
('Thường', 1, 'A', 3),
('Thường', 1, 'A', 4),
('Thường', 1, 'A', 5),
('Thường', 1, 'A', 6),
('Thường', 1, 'A', 7),
('Thường', 1, 'A', 8),
('Thường', 1, 'A', 9),
('Thường', 1, 'B', 1),
('Thường', 1, 'B', 2),
('Thường', 1, 'B', 3),
('Thường', 1, 'B', 4),
('Thường', 1, 'B', 5),
('Thường', 1, 'B', 6),
('Thường', 1, 'B', 7),
('Thường', 1, 'B', 8),
('Thường', 1, 'B', 9),
('Thường', 1, 'C', 1),
('Thường', 1, 'C', 2),
('Thường', 1, 'C', 3),
('Thường', 1, 'C', 4),
('Thường', 1, 'C', 5),
('Thường', 1, 'C', 6),
('Thường', 1, 'C', 7),
('Thường', 1, 'C', 8),
('Thường', 1, 'C', 9),
('Thường', 1, 'D', 1),
('Thường', 1, 'D', 2),
('Thường', 1, 'D', 3),
('Thường', 1, 'D', 4),
('Thường', 1, 'D', 5),
('Thường', 1, 'D', 6),
('Thường', 1, 'D', 7),
('Thường', 1, 'D', 8),
('Thường', 1, 'D', 9),
('VIP', 1, 'E', 1),
('VIP', 1, 'E', 2),
('VIP', 1, 'E', 3),
('VIP', 1, 'E', 4),
('VIP', 1, 'E', 5),
('VIP', 1, 'E', 6),
('VIP', 1, 'E', 7),
('VIP', 1, 'E', 8),
('VIP', 1, 'E', 9),
('VIP', 1, 'F', 1),
('VIP', 1, 'F', 2),
('VIP', 1, 'F', 3),
('VIP', 1, 'F', 4),
('VIP', 1, 'F', 5),
('VIP', 1, 'F', 6),
('VIP', 1, 'F', 7),
('VIP', 1, 'F', 8),
('VIP', 1, 'F', 9),
('Couple', 1, 'G', 1),
('Couple', 1, 'G', 2),
('Couple', 1, 'G', 3),
('Couple', 1, 'G', 4),
('Couple', 1, 'G', 5),
('Couple', 1, 'G', 6),
('Couple', 1, 'G', 7),
('Couple', 1, 'G', 8),
('Couple', 1, 'G', 9),
('Couple', 1, 'H', 1),
('Couple', 1, 'H', 2),
('Couple', 1, 'H', 3),
('Couple', 1, 'H', 4),
('Couple', 1, 'H', 5),
('Couple', 1, 'H', 6),
('Couple', 1, 'H', 7),
('Couple', 1, 'H', 8),
('Couple', 1, 'H', 9),
('Thường', 2, 'A', 1),
('Thường', 2, 'A', 2),
('Thường', 2, 'A', 3),
('Thường', 2, 'A', 4),
('Thường', 2, 'A', 5),
('Thường', 2, 'A', 6),
('Thường', 2, 'A', 7),
('Thường', 2, 'A', 8),
('Thường', 2, 'A', 9),
('Thường', 2, 'B', 1),
('Thường', 2, 'B', 2),
('Thường', 2, 'B', 3),
('Thường', 2, 'B', 4),
('Thường', 2, 'B', 5),
('Thường', 2, 'B', 6),
('Thường', 2, 'B', 7),
('Thường', 2, 'B', 8),
('Thường', 2, 'B', 9),
('Thường', 2, 'C', 1),
('Thường', 2, 'C', 2),
('Thường', 2, 'C', 3),
('Thường', 2, 'C', 4),
('Thường', 2, 'C', 5),
('Thường', 2, 'C', 6),
('Thường', 2, 'C', 7),
('Thường', 2, 'C', 8),
('Thường', 2, 'C', 9),
('Thường', 2, 'D', 1),
('Thường', 2, 'D', 2),
('Thường', 2, 'D', 3),
('Thường', 2, 'D', 4),
('Thường', 2, 'D', 5),
('Thường', 2, 'D', 6),
('Thường', 2, 'D', 7),
('Thường', 2, 'D', 8),
('Thường', 2, 'D', 9),
('VIP', 2, 'E', 1),
('VIP', 2, 'E', 2),
('VIP', 2, 'E', 3),
('VIP', 2, 'E', 4),
('VIP', 2, 'E', 5),
('VIP', 2, 'E', 6),
('VIP', 2, 'E', 7),
('VIP', 2, 'E', 8),
('VIP', 2, 'E', 9),
('VIP', 2, 'F', 1),
('VIP', 2, 'F', 2),
('VIP', 2, 'F', 3),
('VIP', 2, 'F', 4),
('VIP', 2, 'F', 5),
('VIP', 2, 'F', 6),
('VIP', 2, 'F', 7),
('VIP', 2, 'F', 8),
('VIP', 2, 'F', 9),
('Couple', 2, 'G', 1),
('Couple', 2, 'G', 2),
('Couple', 2, 'G', 3),
('Couple', 2, 'G', 4),
('Couple', 2, 'G', 5),
('Couple', 2, 'G', 6),
('Couple', 2, 'G', 7),
('Couple', 2, 'G', 8),
('Couple', 2, 'G', 9),
('Couple', 2, 'H', 1),
('Couple', 2, 'H', 2),
('Couple', 2, 'H', 3),
('Couple', 2, 'H', 4),
('Couple', 2, 'H', 5),
('Couple', 2, 'H', 6),
('Couple', 2, 'H', 7),
('Couple', 2, 'H', 8),
('Couple', 2, 'H', 9),
('Thường', 3, 'A', 1),
('Thường', 3, 'A', 2),
('Thường', 3, 'A', 3),
('Thường', 3, 'A', 4),
('Thường', 3, 'A', 5),
('Thường', 3, 'A', 6),
('Thường', 3, 'A', 7),
('Thường', 3, 'A', 8),
('Thường', 3, 'A', 9),
('Thường', 3, 'B', 1),
('Thường', 3, 'B', 2),
('Thường', 3, 'B', 3),
('Thường', 3, 'B', 4),
('Thường', 3, 'B', 5),
('Thường', 3, 'B', 6),
('Thường', 3, 'B', 7),
('Thường', 3, 'B', 8),
('Thường', 3, 'B', 9),
('Thường', 3, 'C', 1),
('Thường', 3, 'C', 2),
('Thường', 3, 'C', 3),
('Thường', 3, 'C', 4),
('Thường', 3, 'C', 5),
('Thường', 3, 'C', 6),
('Thường', 3, 'C', 7),
('Thường', 3, 'C', 8),
('Thường', 3, 'C', 9),
('Thường', 3, 'D', 1),
('Thường', 3, 'D', 2),
('Thường', 3, 'D', 3),
('Thường', 3, 'D', 4),
('Thường', 3, 'D', 5),
('Thường', 3, 'D', 6),
('Thường', 3, 'D', 7),
('Thường', 3, 'D', 8),
('Thường', 3, 'D', 9),
('VIP', 3, 'E', 1),
('VIP', 3, 'E', 2),
('VIP', 3, 'E', 3),
('VIP', 3, 'E', 4),
('VIP', 3, 'E', 5),
('VIP', 3, 'E', 6),
('VIP', 3, 'E', 7),
('VIP', 3, 'E', 8),
('VIP', 3, 'E', 9),
('VIP', 3, 'F', 1),
('VIP', 3, 'F', 2),
('VIP', 3, 'F', 3),
('VIP', 3, 'F', 4),
('VIP', 3, 'F', 5),
('VIP', 3, 'F', 6),
('VIP', 3, 'F', 7),
('VIP', 3, 'F', 8),
('VIP', 3, 'F', 9),
('Couple', 3, 'G', 1),
('Couple', 3, 'G', 2),
('Couple', 3, 'G', 3),
('Couple', 3, 'G', 4),
('Couple', 3, 'G', 5),
('Couple', 3, 'G', 6),
('Couple', 3, 'G', 7),
('Couple', 3, 'G', 8),
('Couple', 3, 'G', 9),
('Couple', 3, 'H', 1),
('Couple', 3, 'H', 2),
('Couple', 3, 'H', 3),
('Couple', 3, 'H', 4),
('Couple', 3, 'H', 5),
('Couple', 3, 'H', 6),
('Couple', 3, 'H', 7),
('Couple', 3, 'H', 8),
('Couple', 3, 'H', 9),
('Thường', 4, 'A', 1),
('Thường', 4, 'A', 2),
('Thường', 4, 'A', 3),
('Thường', 4, 'A', 4),
('Thường', 4, 'A', 5),
('Thường', 4, 'A', 6),
('Thường', 4, 'A', 7),
('Thường', 4, 'A', 8),
('Thường', 4, 'A', 9),
('Thường', 4, 'B', 1),
('Thường', 4, 'B', 2),
('Thường', 4, 'B', 3),
('Thường', 4, 'B', 4),
('Thường', 4, 'B', 5),
('Thường', 4, 'B', 6),
('Thường', 4, 'B', 7),
('Thường', 4, 'B', 8),
('Thường', 4, 'B', 9),
('Thường', 4, 'C', 1),
('Thường', 4, 'C', 2),
('Thường', 4, 'C', 3),
('Thường', 4, 'C', 4),
('Thường', 4, 'C', 5),
('Thường', 4, 'C', 6),
('Thường', 4, 'C', 7),
('Thường', 4, 'C', 8),
('Thường', 4, 'C', 9),
('Thường', 4, 'D', 1),
('Thường', 4, 'D', 2),
('Thường', 4, 'D', 3),
('Thường', 4, 'D', 4),
('Thường', 4, 'D', 5),
('Thường', 4, 'D', 6),
('Thường', 4, 'D', 7),
('Thường', 4, 'D', 8),
('Thường', 4, 'D', 9),
('VIP', 4, 'E', 1),
('VIP', 4, 'E', 2),
('VIP', 4, 'E', 3),
('VIP', 4, 'E', 4),
('VIP', 4, 'E', 5),
('VIP', 4, 'E', 6),
('VIP', 4, 'E', 7),
('VIP', 4, 'E', 8),
('VIP', 4, 'E', 9),
('VIP', 4, 'F', 1),
('VIP', 4, 'F', 2),
('VIP', 4, 'F', 3),
('VIP', 4, 'F', 4),
('VIP', 4, 'F', 5),
('VIP', 4, 'F', 6),
('VIP', 4, 'F', 7),
('VIP', 4, 'F', 8),
('VIP', 4, 'F', 9),
('Couple', 4, 'G', 1),
('Couple', 4, 'G', 2),
('Couple', 4, 'G', 3),
('Couple', 4, 'G', 4),
('Couple', 4, 'G', 5),
('Couple', 4, 'G', 6),
('Couple', 4, 'G', 7),
('Couple', 4, 'G', 8),
('Couple', 4, 'G', 9),
('Couple', 4, 'H', 1),
('Couple', 4, 'H', 2),
('Couple', 4, 'H', 3),
('Couple', 4, 'H', 4),
('Couple', 4, 'H', 5),
('Couple', 4, 'H', 6),
('Couple', 4, 'H', 7),
('Couple', 4, 'H', 8),
('Couple', 4, 'H', 9),
('Thường', 5, 'A', 1),
('Thường', 5, 'A', 2),
('Thường', 5, 'A', 3),
('Thường', 5, 'A', 4),
('Thường', 5, 'A', 5),
('Thường', 5, 'A', 6),
('Thường', 5, 'A', 7),
('Thường', 5, 'A', 8),
('Thường', 5, 'A', 9),
('Thường', 5, 'B', 1),
('Thường', 5, 'B', 2),
('Thường', 5, 'B', 3),
('Thường', 5, 'B', 4),
('Thường', 5, 'B', 5),
('Thường', 5, 'B', 6),
('Thường', 5, 'B', 7),
('Thường', 5, 'B', 8),
('Thường', 5, 'B', 9),
('Thường', 5, 'C', 1),
('Thường', 5, 'C', 2),
('Thường', 5, 'C', 3),
('Thường', 5, 'C', 4),
('Thường', 5, 'C', 5),
('Thường', 5, 'C', 6),
('Thường', 5, 'C', 7),
('Thường', 5, 'C', 8),
('Thường', 5, 'C', 9),
('Thường', 5, 'D', 1),
('Thường', 5, 'D', 2),
('Thường', 5, 'D', 3),
('Thường', 5, 'D', 4),
('Thường', 5, 'D', 5),
('Thường', 5, 'D', 6),
('Thường', 5, 'D', 7),
('Thường', 5, 'D', 8),
('Thường', 5, 'D', 9),
('VIP', 5, 'E', 1),
('VIP', 5, 'E', 2),
('VIP', 5, 'E', 3),
('VIP', 5, 'E', 4),
('VIP', 5, 'E', 5),
('VIP', 5, 'E', 6),
('VIP', 5, 'E', 7),
('VIP', 5, 'E', 8),
('VIP', 5, 'E', 9),
('VIP', 5, 'F', 1),
('VIP', 5, 'F', 2),
('VIP', 5, 'F', 3),
('VIP', 5, 'F', 4),
('VIP', 5, 'F', 5),
('VIP', 5, 'F', 6),
('VIP', 5, 'F', 7),
('VIP', 5, 'F', 8),
('VIP', 5, 'F', 9),
('Couple', 5, 'G', 1),
('Couple', 5, 'G', 2),
('Couple', 5, 'G', 3),
('Couple', 5, 'G', 4),
('Couple', 5, 'G', 5),
('Couple', 5, 'G', 6),
('Couple', 5, 'G', 7),
('Couple', 5, 'G', 8),
('Couple', 5, 'G', 9),
('Couple', 5, 'H', 1),
('Couple', 5, 'H', 2),
('Couple', 5, 'H', 3),
('Couple', 5, 'H', 4),
('Couple', 5, 'H', 5),
('Couple', 5, 'H', 6),
('Couple', 5, 'H', 7),
('Couple', 5, 'H', 8),
('Couple', 5, 'H', 9),
('Thường', 6, 'A', 1),
('Thường', 6, 'A', 2),
('Thường', 6, 'A', 3),
('Thường', 6, 'A', 4),
('Thường', 6, 'A', 5),
('Thường', 6, 'A', 6),
('Thường', 6, 'A', 7),
('Thường', 6, 'A', 8),
('Thường', 6, 'A', 9),
('Thường', 6, 'B', 1),
('Thường', 6, 'B', 2),
('Thường', 6, 'B', 3),
('Thường', 6, 'B', 4),
('Thường', 6, 'B', 5),
('Thường', 6, 'B', 6),
('Thường', 6, 'B', 7),
('Thường', 6, 'B', 8),
('Thường', 6, 'B', 9),
('Thường', 6, 'C', 1),
('Thường', 6, 'C', 2),
('Thường', 6, 'C', 3),
('Thường', 6, 'C', 4),
('Thường', 6, 'C', 5),
('Thường', 6, 'C', 6),
('Thường', 6, 'C', 7),
('Thường', 6, 'C', 8),
('Thường', 6, 'C', 9),
('Thường', 6, 'D', 1),
('Thường', 6, 'D', 2),
('Thường', 6, 'D', 3),
('Thường', 6, 'D', 4),
('Thường', 6, 'D', 5),
('Thường', 6, 'D', 6),
('Thường', 6, 'D', 7),
('Thường', 6, 'D', 8),
('Thường', 6, 'D', 9),
('VIP', 6, 'E', 1),
('VIP', 6, 'E', 2),
('VIP', 6, 'E', 3),
('VIP', 6, 'E', 4),
('VIP', 6, 'E', 5),
('VIP', 6, 'E', 6),
('VIP', 6, 'E', 7),
('VIP', 6, 'E', 8),
('VIP', 6, 'E', 9),
('VIP', 6, 'F', 1),
('VIP', 6, 'F', 2),
('VIP', 6, 'F', 3),
('VIP', 6, 'F', 4),
('VIP', 6, 'F', 5),
('VIP', 6, 'F', 6),
('VIP', 6, 'F', 7),
('VIP', 6, 'F', 8),
('VIP', 6, 'F', 9),
('Couple', 6, 'G', 1),
('Couple', 6, 'G', 2),
('Couple', 6, 'G', 3),
('Couple', 6, 'G', 4),
('Couple', 6, 'G', 5),
('Couple', 6, 'G', 6),
('Couple', 6, 'G', 7),
('Couple', 6, 'G', 8),
('Couple', 6, 'G', 9),
('Couple', 6, 'H', 1),
('Couple', 6, 'H', 2),
('Couple', 6, 'H', 3),
('Couple', 6, 'H', 4),
('Couple', 6, 'H', 5),
('Couple', 6, 'H', 6),
('Couple', 6, 'H', 7),
('Couple', 6, 'H', 8),
('Couple', 6, 'H', 9),
('Thường', 7, 'A', 1),
('Thường', 7, 'A', 2),
('Thường', 7, 'A', 3),
('Thường', 7, 'A', 4),
('Thường', 7, 'A', 5),
('Thường', 7, 'A', 6),
('Thường', 7, 'A', 7),
('Thường', 7, 'A', 8),
('Thường', 7, 'A', 9),
('Thường', 7, 'B', 1),
('Thường', 7, 'B', 2),
('Thường', 7, 'B', 3),
('Thường', 7, 'B', 4),
('Thường', 7, 'B', 5),
('Thường', 7, 'B', 6),
('Thường', 7, 'B', 7),
('Thường', 7, 'B', 8),
('Thường', 7, 'B', 9),
('Thường', 7, 'C', 1),
('Thường', 7, 'C', 2),
('Thường', 7, 'C', 3),
('Thường', 7, 'C', 4),
('Thường', 7, 'C', 5),
('Thường', 7, 'C', 6),
('Thường', 7, 'C', 7),
('Thường', 7, 'C', 8),
('Thường', 7, 'C', 9),
('Thường', 7, 'D', 1),
('Thường', 7, 'D', 2),
('Thường', 7, 'D', 3),
('Thường', 7, 'D', 4),
('Thường', 7, 'D', 5),
('Thường', 7, 'D', 6),
('Thường', 7, 'D', 7),
('Thường', 7, 'D', 8),
('Thường', 7, 'D', 9),
('VIP', 7, 'E', 1),
('VIP', 7, 'E', 2),
('VIP', 7, 'E', 3),
('VIP', 7, 'E', 4),
('VIP', 7, 'E', 5),
('VIP', 7, 'E', 6),
('VIP', 7, 'E', 7),
('VIP', 7, 'E', 8),
('VIP', 7, 'E', 9),
('VIP', 7, 'F', 1),
('VIP', 7, 'F', 2),
('VIP', 7, 'F', 3),
('VIP', 7, 'F', 4),
('VIP', 7, 'F', 5),
('VIP', 7, 'F', 6),
('VIP', 7, 'F', 7),
('VIP', 7, 'F', 8),
('VIP', 7, 'F', 9),
('Couple', 7, 'G', 1),
('Couple', 7, 'G', 2),
('Couple', 7, 'G', 3),
('Couple', 7, 'G', 4),
('Couple', 7, 'G', 5),
('Couple', 7, 'G', 6),
('Couple', 7, 'G', 7),
('Couple', 7, 'G', 8),
('Couple', 7, 'G', 9),
('Couple', 7, 'H', 1),
('Couple', 7, 'H', 2),
('Couple', 7, 'H', 3),
('Couple', 7, 'H', 4),
('Couple', 7, 'H', 5),
('Couple', 7, 'H', 6),
('Couple', 7, 'H', 7),
('Couple', 7, 'H', 8),
('Couple', 7, 'H', 9),
('Thường', 8, 'A', 1),
('Thường', 8, 'A', 2),
('Thường', 8, 'A', 3),
('Thường', 8, 'A', 4),
('Thường', 8, 'A', 5),
('Thường', 8, 'A', 6),
('Thường', 8, 'A', 7),
('Thường', 8, 'A', 8),
('Thường', 8, 'A', 9),
('Thường', 8, 'B', 1),
('Thường', 8, 'B', 2),
('Thường', 8, 'B', 3),
('Thường', 8, 'B', 4),
('Thường', 8, 'B', 5),
('Thường', 8, 'B', 6),
('Thường', 8, 'B', 7),
('Thường', 8, 'B', 8),
('Thường', 8, 'B', 9),
('Thường', 8, 'C', 1),
('Thường', 8, 'C', 2),
('Thường', 8, 'C', 3),
('Thường', 8, 'C', 4),
('Thường', 8, 'C', 5),
('Thường', 8, 'C', 6),
('Thường', 8, 'C', 7),
('Thường', 8, 'C', 8),
('Thường', 8, 'C', 9),
('Thường', 8, 'D', 1),
('Thường', 8, 'D', 2),
('Thường', 8, 'D', 3),
('Thường', 8, 'D', 4),
('Thường', 8, 'D', 5),
('Thường', 8, 'D', 6),
('Thường', 8, 'D', 7),
('Thường', 8, 'D', 8),
('Thường', 8, 'D', 9),
('VIP', 8, 'E', 1),
('VIP', 8, 'E', 2),
('VIP', 8, 'E', 3),
('VIP', 8, 'E', 4),
('VIP', 8, 'E', 5),
('VIP', 8, 'E', 6),
('VIP', 8, 'E', 7),
('VIP', 8, 'E', 8),
('VIP', 8, 'E', 9),
('VIP', 8, 'F', 1),
('VIP', 8, 'F', 2),
('VIP', 8, 'F', 3),
('VIP', 8, 'F', 4),
('VIP', 8, 'F', 5),
('VIP', 8, 'F', 6),
('VIP', 8, 'F', 7),
('VIP', 8, 'F', 8),
('VIP', 8, 'F', 9),
('Couple', 1, 'G', 1),
('Couple', 8, 'G', 2),
('Couple', 8, 'G', 3),
('Couple', 8, 'G', 4),
('Couple', 8, 'G', 5),
('Couple', 8, 'G', 6),
('Couple', 8, 'G', 7),
('Couple', 8, 'G', 8),
('Couple', 8, 'G', 9),
('Couple', 8, 'H', 1),
('Couple', 8, 'H', 2),
('Couple', 8, 'H', 3),
('Couple', 8, 'H', 4),
('Couple', 8, 'H', 5),
('Couple', 8, 'H', 6),
('Couple', 8, 'H', 7),
('Couple', 8, 'H', 8),
('Couple', 8, 'H', 9),
('Thường', 9, 'A', 1),
('Thường', 9, 'A', 2),
('Thường', 9, 'A', 3),
('Thường', 9, 'A', 4),
('Thường', 9, 'A', 5),
('Thường', 9, 'A', 6),
('Thường', 9, 'A', 7),
('Thường', 9, 'A', 8),
('Thường', 9, 'A', 9),
('Thường', 9, 'B', 1),
('Thường', 9, 'B', 2),
('Thường', 9, 'B', 3),
('Thường', 9, 'B', 4),
('Thường', 9, 'B', 5),
('Thường', 9, 'B', 6),
('Thường', 9, 'B', 7),
('Thường', 9, 'B', 8),
('Thường', 9, 'B', 9),
('Thường', 9, 'C', 1),
('Thường', 9, 'C', 2),
('Thường', 9, 'C', 3),
('Thường', 9, 'C', 4),
('Thường', 9, 'C', 5),
('Thường', 9, 'C', 6),
('Thường', 9, 'C', 7),
('Thường', 9, 'C', 8),
('Thường', 9, 'C', 9),
('Thường', 9, 'D', 1),
('Thường', 9, 'D', 2),
('Thường', 9, 'D', 3),
('Thường', 9, 'D', 4),
('Thường', 9, 'D', 5),
('Thường', 9, 'D', 6),
('Thường', 9, 'D', 7),
('Thường', 9, 'D', 8),
('Thường', 9, 'D', 9),
('VIP', 9, 'E', 1),
('VIP', 9, 'E', 2),
('VIP', 9, 'E', 3),
('VIP', 9, 'E', 4),
('VIP', 9, 'E', 5),
('VIP', 9, 'E', 6),
('VIP', 9, 'E', 7),
('VIP', 9, 'E', 8),
('VIP', 9, 'E', 9),
('VIP', 9, 'F', 1),
('VIP', 9, 'F', 2),
('VIP', 9, 'F', 3),
('VIP', 9, 'F', 4),
('VIP', 9, 'F', 5),
('VIP', 9, 'F', 6),
('VIP', 9, 'F', 7),
('VIP', 9, 'F', 8),
('VIP', 9, 'F', 9),
('Couple', 9, 'G', 1),
('Couple', 9, 'G', 2),
('Couple', 9, 'G', 3),
('Couple', 9, 'G', 4),
('Couple', 9, 'G', 5),
('Couple', 9, 'G', 6),
('Couple', 9, 'G', 7),
('Couple', 9, 'G', 8),
('Couple', 9, 'G', 9),
('Couple', 9, 'H', 1),
('Couple', 9, 'H', 2),
('Couple', 9, 'H', 3),
('Couple', 9, 'H', 4),
('Couple', 9, 'H', 5),
('Couple', 9, 'H', 6),
('Couple', 9, 'H', 7),
('Couple', 9, 'H', 8),
('Couple', 9, 'H', 9);

create table  phongchuaghe(
	id int not null auto_increment,
    id_ghe int,
    id_phong int,
    primary key(id),
    foreign key(id_phong) references phong(id),
	foreign key(id_ghe) references ghengoi(id)
);

INSERT INTO phongchuaghe (id,id_ghe,id_phong) VALUES 
(1,1,2),
(2,2,3),
(3,3,5),
(4,4,4),
(5,5,7),
(6,6,6),
(7,7,8),
(8,8,9),
(9,9,1);

create table  datve(
	id int not null auto_increment,
	id_makhachhang int,
	id_lichchieuphim int, 
    id_phongchuaghe int,
    gia double,
    trangthai_ghe int,
	primary key(id),
	foreign key(id_makhachhang) references khachhang(id),
    foreign key(id_lichchieuphim) references lichchieupphim(id),
    foreign key(id_phongchuaghe) references phongchuaghe(id)
    
);

INSERT INTO datve (id_makhachhang, id_lichchieuphim, id_phongchuaghe, gia, trangthai_ghe) VALUES
(1, 1, 4, 750000, 1),
(2, 2, 2, 750000, 0),
(3, 4, 5, 75000, 1),
(4, 6, 6, 90000, 0),
(5, 5, 3, 85000, 0),
(6, 7, 1, 105000, 1);












